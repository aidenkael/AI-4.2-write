...
当前 BookDistill v0.1.1 的 evidence-first、来源 fingerprint、章节/行号校验等属于值得保留的地基，但 `model.md + mechanisms.md` 不是最终知识资产形态。

长期目标是 Book Knowledge Package（BKP）：一本书经 1 次全维度基础蒸馏 + 1～N 次有价值的专项深挖后，形成可长期检索的固定知识资产；日常正式写作通常只检索 BKP，不为当前场景重新蒸馏整本原著。
...
在 BKP / BookProfile / BookDistill vNext 经验证前，禁止批量蒸馏几十本素材或冻结最终 schema。
...
5. **禁止未经授权执行：** `git reset --hard`、`git restore .`、`git checkout -- .`、`git clean -fd` 或任何批量 restore / clean。
...
准入：项目总体说明、长期规则、能力地图、门禁、项目记忆、权威索引等。单轮实验/临时 Agent 文件默认不放这里。
...
已结束的 Benchmark、被新版替代但仍需追溯的文件、一次性有参考价值的报告。
...
2. 单次任务只推进当前门禁中的一个明确子目标；发现额外问题可以记录，不得“顺手扩展”。
3. 候选池出现过某项目，不等于已比较/验证；必须区分正式实测、轻量实测、代码/架构审阅、未实测。
4. 新 Benchmark、重大目录变化、批量处理、新架构建设、跨门禁动作均需要用户明确授权。
5. 聊天建议与长期手册/当前索引/项目记忆/门禁或用户最新决定冲突时，先停止并对齐。
6. 不得为了“赶 Gate”跳过已确认的成熟方案复查、关键协议设计或作者确认。

## 10. 工作台架构规则

- 能力地图 C01–C20 是路由地图，不是一一对应的作者 Skill 清单。
...
1. 根目录 `AI-write_长期开发手册.md`（尤其 AUTO 当前状态/下一步/风险区）；
2. `00_项目控制/当前工作索引.md`；
3. `00_项目控制/项目推进记忆.md`；
4. `00_项目控制/项目阶段门禁.md`；
5. `AGENTS.md`（仅长期规则变化时）；
6. 相关专项 STATUS / provenance。
