...
FINALIZED settlement output allowlist 验证（BD_SETTLEMENT_CURRENT_BOOK_ONLY）：
  - 当前作品 subtree（02_素材知识库/<book_id>_<work_name>/ 下 bkp / knowledge / meta）→ 允许
  ...
  - 其他作品目录 / 伪装前缀文件 → 禁止
  - 非法 book_id / 绝对路径 / .. / 非 02_素材知识库 路径 → ValueError
...
def _al_dir() -> list[str]:
    ...
    return sc.build_settlement_allowlist_from_dir("02_素材知识库/book_0001_Alpha")
...
def test_current_book_subtree_allowed():
    for al in (_al(), _al_dir()):
        assert sc.is_settlement_allowed("02_素材知识库/book_0001_Alpha/bkp/identity.json", al) is True
        assert sc.is_settlement_allowed("02_素材知识库/book_0001_Alpha/knowledge/世界观.md", al) is True
        assert sc.is_settlement_allowed("02_素材知识库/book_0001_Alpha/metadata.json", al) is True
...
def test_allowlist_shape_has_no_whole_distill_dir():
    """allowlist 绝不含整个 02_素材知识库/ 目录授权。"""
    for al in (_al(), _al_dir()):
        assert "02_素材知识库/" not in al
        assert "02_素材知识库" not in al
        assert any(a.startswith("02_素材知识库/") for a in al)  # 但含当前作品前缀
...
def test_sibling_book_not_allowed():
    ...
    for al in (_al(), _al_dir()):
        assert sc.is_settlement_allowed("02_素材知识库/book_0002_Beta/bkp/identity.json", al) is False
        assert sc.is_settlement_allowed("02_素材知识库/book_0002_Beta/metadata.json", al) is False


def test_prefix_spoof_not_allowed():
    """伪造前缀（book_00010 / book_0001_Alpha_evil）不得放行。"""
    for al in (_al(), _al_dir()):
        assert sc.is_settlement_allowed("02_素材知识库/book_00010_Fake/bkp/identity.json", al) is False
        assert sc.is_settlement_allowed("02_素材知识库/book_0001_Alpha_evil/bkp/x.md", al) is False


def test_whole_distill_dir_default_not_allowed():
    """模块默认 allowlist（不带参数）不含任何蒸馏目录授权。"""
    assert sc.is_settlement_allowed("02_素材知识库/book_0001_Alpha/bkp/identity.json") is False
    assert sc.is_settlement_allowed("02_素材知识库/anything.md") is False
...
def test_spoofed_prefix_files_not_allowed():
    ...
    # 相似目录名不得放行（非 02_素材知识库 前缀）
    assert sc.is_settlement_allowed("02_素材知识库x/book_0001/x.md", _al()) is False


def test_mixed_allowed_and_never_stage_consistency():
    # 允许面与 never-stage 不重叠：允许的都必须能通过第二道过滤
    for al in (_al(), _al_dir()):
        for rel in ("02_素材知识库/book_0001_Alpha/bkp/identity.json",
                    "01_原始素材/素材资产.json"):
            assert sc.is_settlement_allowed(rel, al) is True
            assert sc.is_settlement_never_stage(rel) is False
...
def test_work_name_injection_rejected():
    # .. / 绝对路径 / 路径分隔符 注入 → sanitize 后不再逃逸 02_素材知识库
    al = sc.build_settlement_allowlist("book_0001", "../../evil")
    prefix = next(a for a in al if a.startswith("02_素材知识库/"))
    assert prefix.startswith("02_素材知识库/book_0001_")
    assert ".." not in prefix
    # 结构恰为 02_素材知识库/<book_id>_<safe>/（2 个 /：目录层级 + 结尾），无跨目录注入
    assert prefix.count("/") == 2
    # 绝对路径 work_name 同样被 sanitize
    al2 = sc.build_settlement_allowlist("book_0001", "C:/Windows/evil")
    assert any(a.startswith("02_素材知识库/book_0001_") for a in al2)
...
def test_from_dir_path_validation():
    ...
    for bad in ("/02_素材知识库/book_0001_Alpha",
                ...
                "02_素材知识库/book_1_Alpha",
                "02_素材知识库/alpha_Alpha"):
        with pytest.raises(ValueError):
            sc.build_settlement_allowlist_from_dir(bad)
    # 合法输入可正常构建
    al = sc.build_settlement_allowlist_from_dir("02_素材知识库/book_0035_长安十二时辰")
    assert "02_素材知识库/book_0035_长安十二时辰/" in al
    assert len(al) == 4
