...
Each local Reader reads exactly one batch's real prose, writes free Literary
Discovery, then audits the six domains and selectively projects observations;
all layers are atomically published in one source-bound note. In parallel, one dedicated
...
- **Reader 只做**：完整读取一个 batch 全部 span 的原文 → 自由 Literary Discovery → 六域 Coverage Audit → 选择性 Structured Projections → 同一 temp note 全文 `note-publish` 原子发布 canonical note。先读懂再整理，六域不是首次阅读 checklist；每域 checked 前必须回看当前 Literary Discovery、再回看本批原文，主动寻找能反证 Discovery 理解充分性的显著现象，找到后先补回 Discovery。checked 只证明做过反证检查，不能用“已涉及/基本覆盖/无需补充”自我认证。Structured Projections 前还须做一次开放的读者作用反查，但它不是 taxonomy 或固定第七域，严肃文学不得硬造类型机制。自由发现不要求 dimension、单句、mechanism 或数量配额；没有高价值发现可以为空，但只能是完整阅读与反证回查后仍无值得保存的发现，不能因网文/类型文学/表面直接/技巧常见而降低分析深度；0 个 structured Observation + 有自由 Discovery 合法。Reader 绝不 `reading-commit`、绝不改 manifest/ledger/convergence/BKP/acceptance/response、绝不生成知识卡、绝不再分派子 Agent。
...
3. 并行阅读循环（局部阅读权威 = manifest/ledger；连续首读权威 = `_work/reader_continuity/state.json`；**动态补位，不做固定 wave barrier**）。保持 continuity worker 和 local Readers 同时运行，重复直到 ledger 全部 completed 且 continuity state `complete=true`：
   ...
   b. 对每个 dispatch，用 Agent 工具以 `subagent_type="{READER_SUBAGENT}"` 启动**一个** Reader，只交给它这一个 batch 的分派载荷（staging_dir、sp_dir、batch_id、spans、temp_note_path、note_template、note_publish_command、request_id/run_id/manifest_hash/source_fingerprint）。Reader 会**直接阅读该 batch 全部 span 的完整原文**（不抽样、不只读首部、不伪造 scan_refs、不凭记忆替代），先完成自由 Literary Discovery，之后才做六域 checked（故事与大纲 / 人物与关系 / 章节与场景 / 冲突与节奏 / 世界与题材 / 语言与读者体验；每域 `0 findings` 合法、“未检查”不合法）。每域 checked 前必须回看当前 Literary Discovery、再回看本批原文并主动寻找能证明 Discovery 对该域理解不充分的显著现象；发现后先补回 Discovery，checked 只表示反证检查已执行，禁止用“已涉及/基本覆盖/无需补充”替代实际核对。六域之后、Structured Projections 之前，再开放反查什么具体文本作用让目标读者继续读、获得回报、改变预期或重新理解人物/局面；这不是 taxonomy 或固定第七域，不要求逐项回答，也不得给严肃文学硬造类型/连载机制。没有高价值发现可以为空，但只能是完整阅读并完成反证回查后仍无值得保存的发现，不得因网文、类型文学、表面直接或技巧常见而少分析。最后仅将不明显损失含义的发现投影为 structured Observation，保留自由段落并运行 `note-publish` 全文原子发布 canonical note。尽量保持在飞 Reader 接近上限以最大化吞吐，但同一时间所有书合计 active Reader 永远 `<= {READER_LIMIT}`。
   ...
   e. rolling convergence：逐批实际读取已处理的 canonical batch note 全文，将 `Literary Discovery` 与 `Structured Projections` 作为并列 discovery input，不能只提取 Observation 行。持续维护 `{bd}/_work/convergence_state.md`（滚动收敛状态，过程工件，**绝不进入 02**）：processed batch ids、mechanism clusters、accumulated evidence、conflicts/counterevidence、scope/boundary differences、unresolved questions、canonical candidates、supporting candidates；对尚未能安全压缩的自由发现保留语境、矛盾解释与 canonical note 路径/区块定位，供 final 回读，不因未转成 Observation 而丢弃，不自动全部提升成 Mechanism，不按文字相似强制去重。**优先持续补满 Reader，绝不让收敛把并行阅读重新串行化。**
...
4. 全部 ledger completed 且 continuity spine complete 后，必须再做一次**全书 final Editorial Convergence**（不能只用滚动中间态）：按 manifest 有界逐批重新读取每份 canonical batch note 的 `Literary Discovery` 与 `Structured Projections`，两者是并列 discovery input；不能用滚动摘要或 Observation 列表替代自由段落，不因未转成 Observation 而丢弃，不自动全部提升成 Mechanism。交叉验证、合并同质项、降级单章小技巧，并将 continuity history/rolling state 作为一类不可替代的顺序阅读输入，保留反证/边界。归并只在 conditions / mechanism / scale / effect 四者语义实质等价时进行，绝不按文字相似去重；归并后保留全部来源 evidence 与 merged_from 关系，不得丢失 scope/boundary/counterevidence。无法确认等价时宁可分开。形成：
...
以下 Observer 合同提供分析视角；其中 dimension / 单句条目格式只约束选择性结构化投影及既有 observer bridge 输出，不要求 local Reader 把自由 Literary Discovery 全部转成条目，也不能成为首次阅读清单。continuity 继续只维护自然语言 experience_update + rolling_state，不套用 local note schema。
