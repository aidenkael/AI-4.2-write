...
FINALIZED settlement 允许进 Git 的 tracked 面（Phase 2B2.1 起按当前作品动态构建）：
  - 02_原著蒸馏/<book_id>_<work_name>/  当前作品的单一 distillation subtree
...
绝不进 settlement：
  - 任何 sibling / 伪造前缀作品目录（02_原著蒸馏/ 整目录授权已废止）
...
# 兼容默认：不含任何蒸馏目录的静态面（禁止整个 02_原著蒸馏/ 授权）
...
def distill_dir_prefix(book_id: str, work_name: str) -> str:
    """构造当前作品 distillation subtree 前缀 `02_原著蒸馏/<book_id>_<work_name>/`。
    ...
    return f"02_原著蒸馏/{book_id}_{safe}/"
...
def build_settlement_allowlist(book_id: str, work_name: str) -> list[str]:
    ...
    结果形如：
      ["02_原著蒸馏/book_0035_长安十二时辰/",
       "01_原始素材/素材资产.json",
       "01_原始素材/素材清单.csv",
       "01_原始素材/素材总索引.md"]

    绝不允许整个 02_原著蒸馏/ 目录（BD_SETTLEMENT_CURRENT_BOOK_ONLY）。
    """
...
def build_settlement_allowlist_from_dir(distill_rel: str) -> list[str]:
    """由蒸馏 subtree 相对路径（如 `02_原著蒸馏/book_0001_Alpha`）构建 allowlist。

    校验（BD_SIBLING_BOOK_CHANGE_UNEXPECTED）：
      - distill_rel 必须是 `02_原著蒸馏/` 下的相对 posix 路径（拒绝绝对路径 / .. / 其他目录）
    ...
    parts = rel.split("/")
    if len(parts) != 2 or parts[0] != "02_原著蒸馏":
        raise ValueError(
            f"distill 路径必须位于 02_原著蒸馏/ 下且为单层目录: {distill_rel!r}")
    base = parts[1]
    m = BOOK_ID_PREFIX_RE.match(base)
    if not m:
        raise ValueError(f"distill 目录名必须以合法 <book_id>_ 开头: {base!r}")
    return [f"02_原著蒸馏/{base}/", *MATERIAL_STATE_FILES]
