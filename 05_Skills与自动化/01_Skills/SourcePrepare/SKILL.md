...
有 EPUB 的作品，按 **EPUB-first** 选源：

1. 先只检查/转换 EPUB（EPUB 14 项质检 + Pandoc 转换）；
2. 只要**至少一个 EPUB** 通过完整性验证并得到 `PASS`，就**直接使用 EPUB**；
3. 此时**不再读取/转换 TXT/PDF** 来与 EPUB 比长度，也**不能因为 TXT 字符更多而反选 TXT**；
4. 只有**没有任何 EPUB PASS**（EPUB 无法读取 / 损坏 / 转换失败 / 质检未通过）时，才 fallback 到 TXT/PDF；
5. TXT 保留、不删除，它只是 EPUB 的兜底来源；
6. **没有 EPUB 的作品**，继续按现有 TXT/PDF 逻辑处理；
7. **多个 EPUB** 时，在同一 EPUB 池内按现有质量指标选优；
8. **不针对《亮剑》等具体作品写特例。**

EPUB 池内选优仍沿用（仅作用于 EPUB 池内）：

- **单 EPUB 只要通过质检即可 PASS**，不要求必须有 TXT 伴生；
- 若某 EPUB 正文长度仅为池内最长 EPUB 的 70% 以下，标记“可能不完整”并降为 `REVIEW`；
- **近邻选源修正**：当两个候选正文长度差 ≤ 2%（即较短/较长 ≥ 98%）时，若较长来源为 `REVIEW` 且未识别章节（0 章），而另一来源为 `PASS` 且有章节，则优先选择 `PASS` + 有章节的来源（避免丢失分章、整体降级）。修正理由记录在选中来源的“单文件备注”，不因此把整体状态升为 `REVIEW`。
...
4. 选 1 部 EPUB + TXT 多格式作品实测（确认 EPUB PASS 时选 EPUB、不再因 TXT 更长反选 TXT）；
