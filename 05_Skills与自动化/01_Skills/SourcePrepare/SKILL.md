...
- 每本拆分单书在 `01_原始素材/素材清单.csv` 的**「来源容器」列**写入合集名（如 `马伯庸作品合集`），
  空值表示独立来源。
...
- 一个合集只占一个物理目录名额：拆出的单书 EPUB、`collection_manifest.json` 与原始合集 EPUB 同放该目录；原始合集永久保留，SP 不修改任何 EPUB。
