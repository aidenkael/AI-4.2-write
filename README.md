...
| `03_作品工程` | 具体作品的写作工程 | ✅ |
| `04_写作知识库` | 长篇结构与剧情推进、人物与群像、人物关系·对白·场景、悬念伏笔与信息控制、情绪爽点与读者驱动力、世界观制度与社会运行（含「古今制度转换」子目录）、文风叙事与语言表达、失败案例与反面经验 | ✅ |
...
普通单本作品一个物理目录：`<分类>/<作品>/`，目录内直接放源文件（EPUB/TXT/PDF），
**不再嵌套 `00_原始文件`**。合集/套装允许“一个目录含多部作品”，拆分出的每本独立 EPUB
通过 `collection_manifest.json` 与索引「来源容器」列登记（详见 [`目录规范.md`](目录规范.md) 原则 4）。
每部作品有稳定 ID `book_XXXX`（见索引），**身份以 book_id + 中央索引为准，不以文件夹名决定**。
...
- `00_项目控制/原始素材清单.csv`：给 Agent / 自动化，一部作品可对应多条文件记录，
  含作品ID、作者、格式、SHA256、是否主来源、**来源容器**、SourcePrepare 状态等 22 列。
  「来源容器」非空时表示该文件来自某个合集（如 `马伯庸作品合集`），空表示独立来源。
