...
**旧 mapping 作废（`PRE_RUN_MAPPING_EXPOSURE`）**：此前记录在 tracked 文件中的 condition→A/B/C 对应表已在正式运行前暴露给作者，自 2026-08-10 起**永久作废**，不得用于 Round2A 正式作者盲评。

- 作废发生在任何正式 output 生成之前；
- 未查看任何正式结果；
- 不涉及根据结果重新随机；
- 不改变 D0/M1/M2、任务、重复次数或评审问题。

正式运行的匿名映射约束（不随作废改变）：

- 每个评审组（G1–G4）均为 D0 / M1 / M2 各一次；
- 每个 condition 在 A/B/C 位置分布为 2/1/1；
- 同一 condition 不在相邻两组保持同一字母；
- 与已作废旧映射四组均不同。

作者盲评实际 mapping 在正式运行前独立生成并 Local Only 封存（`blind_map_presealed_r2a.json`，SHA256 见 `B02_G_Round2A_blind_map_seal_v0.1.md`）；作者全部评审完成前不得公开。实际映射内容不写入任何 tracked 文件。
