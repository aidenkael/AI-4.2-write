...
> 当前正式门禁：**G2**（尚未退出；不得自动进入 G3）。  
> 当前战略状态：**暂停扩大原著蒸馏数量，先重新审查 GitHub 能力、定义完整原著知识资产与 BookDistill vNext，再决定规模化。**
...
## Phase C｜BookDistill vNext 最小实现

目标：

- 复用当前 evidence-first 地基；
- 增加全维度基础蒸馏；
- 基础蒸馏后生成 BookProfile；
- 允许按优势/不确定项做多次专项蒸馏；
- 最终封装 BKP；
...
- 正式 Gate：G2
- 已完成真实 BookDistill：`book_0038 一九八四`、`book_0065 三体`
- 当前判断：BookDistill evidence-first 地基有效，但作者综合层压缩过强，尚不足以进入大规模蒸馏。
- 当前主任务：重新审查完整工作台相关 GitHub 能力；定义 BKP / BookProfile / BookDistill vNext。
- 当前明确不做：第三本自动蒸馏、60 本批量、正式 G3、完整 UI、最终知识库冻结。
<!-- AUTO:CURRENT_STATE END -->

<!-- AUTO:NEXT_ACTIONS START -->
## 下一步候选动作

1. 做一次“完整工作台能力层 GitHub 再审查”；
2. 输出每能力层：直接借 / 改造 / 自研缺口；
3. 基于审查结果定义 BKP v0.1 候选协议；
4. 定义 BookProfile 的全维度基础评价 + 置信度 + 专项预算规则；
5. 再决定 BookDistill vNext 实现，不提前编码。
