...
这里的“自研”指**掌握架构、知识协议、数据边界、集成方式和验收标准**，不等于每个具体能力都由 ChatGPT 或本项目重新实现。具体实现可以直接来自成熟开源项目，也可以由最适合当前任务的 ChatGPT、Agent 或人工完成；只要最终接口、数据和边界收敛进 AI-write 的统一体系即可。
...
默认**不固定必须由 ChatGPT、Agent 或用户本人完成任务**。用户明确指定执行者时优先遵从；用户未指定时，由系统根据任务性质直接选择成功率高、操作简单、成本合理的执行方式，不把执行者选择重新丢给作者。
...
- **混合任务**：可以由 ChatGPT 锁定目标、协议、验收标准和最终审查，由 Agent 本地执行；但这不是强制流程，如果单一执行者能更简单可靠地完成，就不要为了“分工”额外拆任务；
...
> **AI-write 不从零重造 AI 写作系统：大量吸收成熟项目，少量自研架构连接层、知识协议和作者控制边界，具体任务交给最适合的执行者，把最好的现成能力组成一个真正服务长期写作的工作台；当前先完成其中最前面的“原著学习能力”。**