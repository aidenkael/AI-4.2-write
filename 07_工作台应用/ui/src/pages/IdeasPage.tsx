...
export function IdeasPage() {
  const { state, actions } = useApp(); const desk=useIllustration('desk'); const [value,setValue]=useState(''); const [filter,setFilter]=useState('全部')
  return <div className="page"><PageHeader title="灵感箱" subtitle="随时记录闪过的想法，为创作积蓄灵感能量。" art={desk}/>
    <section className="panel idea-composer"><h3><Sparkles/>快速记录灵感</h3><textarea value={value} onChange={(e)=>setValue(e.target.value)} placeholder="此刻的想法、场景或对白…" maxLength={1000}/><small>{value.length}/1000</small><div><button className="active"><Type/>文字</button><button><Link/>链接</button><button><FileText/>文件</button><button className="primary" onClick={async()=>{await actions.createIdea(value);setValue('')}} disabled={!value.trim()}><PenLine/>记录灵感</button></div></section>
    <div className="idea-filters">{['全部','尚未使用','已用于作品'].map(x=><button key={x} className={filter===x?'active':''} onClick={()=>setFilter(x)}>{x}</button>)}</div>
    <section className="ideas-grid">{state.ideas.filter(i=>filter==='全部'||(filter==='已用于作品'?i.used:!i.used)).map((idea)=>{const Icon=kinds[idea.kind];return <article className="panel idea-card" key={idea.id}><header><span><Icon/>{idea.kind}</span><time>{idea.time}</time></header><p>{idea.content}</p><small><Sparkles/>AI 小提示：{idea.note}</small><footer><button>♡ 先留着</button><button><PenLine/>帮我发展</button><button onClick={()=>actions.toggleIdea(idea.id)}><Plus/>{idea.used?'移出作品':'加入作品'}</button></footer></article>})}</section>
  </div>
}
