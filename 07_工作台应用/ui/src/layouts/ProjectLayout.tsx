...
export function ProjectLayout({ children }: { children: ReactNode }) {
  const { state, actions } = useApp(); const city = useIllustration('city')
  return <div className="project-shell">
    <header className="projectbar" style={{ backgroundImage: `linear-gradient(90deg,#fff 70%,rgba(255,255,255,.25)),url(${city})` }}>
      <button className="project-title" onClick={() => actions.navigate('projects')}><ArrowLeft/>迷雾之城</button>
      <span className="autosave"><CircleCheck size={15}/> 自动保存中&nbsp; 14:32:05</span>
      <nav aria-label="作品内导航"><button onClick={() => actions.navigate('projects')}>作品概览</button>{items.map((item) => <button key={item.id} className={state.projectSection === item.id ? 'active' : ''} onClick={() => actions.setProjectSection(item.id)}>{item.label}</button>)}</nav>
    </header>
