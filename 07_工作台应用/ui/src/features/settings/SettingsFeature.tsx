...
export function SettingsFeature() {
  ...
  return <div className="page settings-page"><PageHeader title="设置" subtitle="配置更新作品状态、创作任务执行与本次会话的界面通知。" art={editorialIllustrations.ideasSettings} /><div className="settings-layout"><aside className="panel settings-menu">{menu.map(({ label, Icon }) => <button key={label} className={section === label ? 'active' : ''} onClick={() => setSection(label)}><Icon/>{label}</button>)}</aside>
    <section className="panel settings-content">
      ...
      {section === '界面与通知' ? <><div className="section-title"><div><h2>界面与通知</h2><p>通知偏好只在本次 Go Write 会话中生效。</p></div></div><div className="settings-group session-preferences"><div className="settings-group-title"><h3>通知</h3><p>任务完成或失败时显示全局通知；提示音使用本机音量。</p></div><label><input type="checkbox" checked={state.preferences.sound} onChange={(event) => actions.setPreference('sound', event.target.checked)}/> 在执行完成/失败时播放提示音</label></div></> : null}
