import { Bot, Cloud, Folder, Image, RefreshCw, Settings as SettingsIcon, SlidersHorizontal } from 'lucide-react'
...
export function SettingsFeature() {
  ...
  return <div className="page"><PageHeader title="设置" subtitle="管理应用配置、AI 服务与个性化偏好" art={city}/><div className="settings-layout"><aside className="panel settings-menu">{menu.map(({ label, Icon }) => <button key={label} className={section === label ? 'active' : ''} onClick={() => setSection(label)}><Icon/>{label}</button>)}<div style={{ backgroundImage: `url(${desk})` }}/></aside>
    <section className="panel settings-content"><div className="section-title"><div><h2>{section}</h2><p>{section === '执行配置' ? '选择真实可用的交互桥或直接执行环境。' : '设置只显示本机实际发现的信息。'}</p></div>{section !== '插图与视觉' ? <button onClick={controller.refresh} disabled={controller.loading}><RefreshCw/>{controller.loading ? '检测中…' : '刷新状态'}</button> : null}</div>
      ...
      {section === 'AI 服务 / API' && controller.data ? <div className="real-api-settings"><div className="api-summary"><Cloud/><div><h3>本机发现的执行能力</h3><p>模型只来自可执行的本机 Agent；不会接受任意模型名或 API 地址。</p></div></div>{controller.agents.map((agent) => <article key={agent.agent_id}><strong>{agent.display_name}</strong><span>{agent.direct.model_selection === 'managed' ? `受管模型：${agent.direct.managed_model?.display_name ?? '未检测'}` : agent.direct.model_selection === 'selectable' ? `可选模型：${agent.direct.models.length}` : '无可执行模型'}</span><em>{agent.direct.available ? '可用' : '不可用'}</em></article>)}</div> : null}
